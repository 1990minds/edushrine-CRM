...
import { crmSelector, fetchAllCrm, fetchOneCrm } from '../../api/crm'
...
<div class="p-2 px-8 w-full">
        <div class="relative">
          ...
          <input class="w-full md:w-full h-9 py-1 pl-9 pr-4 text-sm text-coolGray-500 font-medium placeholder-coolGray-400 focus:outline-none focus:ring-2 focus:ring-opacity-50 border border-coolGray-200 rounded-lg shadow-xsm" type="text" placeholder="Search"/>
        </div>
      </div>
            
      <div class="w-full md:w-full xl:w-1/2 px-8 mb-3">
      <Select bg='white' color='grey' placeholder='Filter Campus'>
        {
          branches?.map((item,i)=>{
            return(
      ...
      <Modal size='xs' isOpen={isOpen} onClose={onClose}>
        ...
        <ModalContent>
          ...
          <ModalBody>
            ...
            {/* <Lorem count={2} /> */}
            <Select mt='20px' placeholder='status' onChange={(e)=>setStatus(e.target.value)}>
            <option value='New'>New</option>
            <option value='Intrested'>Intrested</option>
            <option value='Not Intrested'>Not Intrested</option>
            </Select>
            <Select mt='20px'  placeholder='status' onChange={(e)=>setComment(e.target.value)} >
              {
                status==='Intrested'?
                <>
